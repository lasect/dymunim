...
export async function enqueueJob(jobType: string, payload: Record<string, unknown> = {}, priority: number = 0): Promise<number> {
...
export async function listJobs(status?: string, jobType?: string, limit: number = 50, offset: number = 0): Promise<Job[]> {
  return db.query<Job>(
    'SELECT * FROM list_jobs($1, $2, $3, $4)',
...
export async function listJobTypes(): Promise<{ job_type: string; handler: string; timeout_seconds: number; max_retries: number }[]> {
  return db.query('SELECT job_type, handler, timeout_seconds, max_retries FROM jobs_registry ORDER BY job_type');
}

export async function deleteJob(jobId: number): Promise<boolean> {
  const result = await db.execute('DELETE FROM jobs_state WHERE job_id = $1', [jobId]);
  return result > 0;
}
